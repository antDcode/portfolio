...
$o('#about', (e) => {
    e.intersectionRatio === 1 && $('#about .counters').classList.add('active');
})
...
function getPoints() {
    ...
    for (const el of $$('.decor-balls, .five-circle, #five-steps')) {
...
$e(window, 'scroll', () => {
    ...
    for (const id in points) {
        ...
        if (scrollY > point.start && scrollY < point.end) {
            ...
            if (id === 'five-steps') {
                p = Math.max(0, p - 0.3) * 2;
                $('.sq>ul').style.translate = +Math.max(-58, -p * 100) + '% 0';
...
$e(body, 'click', e => {
    ...
    if (fx.startsWith('fx-')) {
        ...
        } else if (fx === 'fx-mr-about-mentor') {
            el.remove();
            $(".about-text-box").classList.add("active")
        }
    }
